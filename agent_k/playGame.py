...
class PlayGame:
    ...
    def get_evaluation(self, piece_list: list, color: str) -> tuple:
        ...
        if color == constant.BLACK:
            for curr_piece in piece_list:
                ...
                if j == 0 or j == 7:
                    safe_piece_count += 1
                if self.board[i][j] == constant.BLACK_PIECE:
                    dist_from_promo += i
                protection = [[-1, -1], [-1, 1]]
            ...
            for curr_piece in piece_list:
                ...
                if 3 <= i <= 4:
                    middle_two_rows += 1
                if self.board[i][j] == constant.WHITE_PIECE:
                    dist_from_promo += (7 - i)
                if i < 4:
                    advanced_pawn_count += 1
                if j == 0 or j == 7:
        ...
        return (back_row, middle_box, middle_two_rows, protected_cnt, vulnerable_cnt, dist_from_promo, advanced_pawn_count, safe_piece_count)
    ...
    def eval_core(self) -> float:
        ...
        (black_back_row, black_middle_box, black_middle_two_rows, black_protected_piece, black_vulnerable_piece, black_dist_from_promo, black_advanced_pawn_count, black_safe_cnt) = \
            self.get_evaluation(self.black_piece_list, constant.BLACK)
        (white_back_row, white_middle_box, white_middle_two_rows, white_protected_piece, white_vulnerable_piece, white_dist_from_promo, white_advanced_pawn_count, white_safe_cnt) = \
            self.get_evaluation(self.white_piece_list, constant.WHITE) 
        
        if black_piece_count > 0:
            black_aggregated_dist_from_promo = (black_dist_from_promo / black_piece_count)
            black_aggr_advanced_pawn = (black_advanced_pawn_count / black_piece_count)
            black_aggr_safe_cnt = (black_safe_cnt / black_piece_count)
        else:
            black_aggregated_dist_from_promo = 0
            black_aggr_advanced_pawn = 0
            black_aggr_safe_cnt = 0
        if white_piece_count > 0:
            white_aggregated_dist_from_promo = (white_dist_from_promo / white_piece_count)
            white_aggr_advanced_pawn = (white_advanced_pawn_count / white_piece_count)
            white_aggr_safe_cnt = (white_safe_cnt / white_piece_count)
        else:
            white_aggregated_dist_from_promo = 0
            white_aggr_advanced_pawn = 0
            white_aggr_safe_cnt = 0

        if self.get_max_player == constant.BLACK:
            white_piece_eval = (5 * white_regular_piece) + (7.75 * white_king_piece) + (4 * white_back_row) + (0.5 * white_middle_two_rows) + (0.5 * white_middle_box) + (-3 * white_vulnerable_piece) + (4 * white_protected_piece) + (white_aggr_advanced_pawn * 3) + (white_aggr_safe_cnt * 4)
            black_piece_eval = (5 * black_regular_piece) + (7.75 * black_king_piece) + (4 * black_back_row) + (0.5 * black_middle_two_rows) + (0.5 * black_middle_box) + (-3 * black_vulnerable_piece) + (4 * black_protected_piece) + (black_aggr_advanced_pawn * 3) + (black_aggr_safe_cnt * 4)
        else:
            white_piece_eval = (5 * white_regular_piece) + (7.75 * white_king_piece) + (4 * white_back_row) + (0.5 * white_middle_two_rows) + (0.5 * white_middle_box) + (-3 * white_vulnerable_piece) + (4 * white_protected_piece) + (white_aggr_advanced_pawn * 4) + (white_aggr_safe_cnt * 6)
            black_piece_eval = (5 * black_regular_piece) + (7.75 * black_king_piece) + (4 * black_back_row) + (0.5 * black_middle_two_rows) + (0.5 * black_middle_box) + (-3 * black_vulnerable_piece) + (4 * black_protected_piece) + (black_aggr_advanced_pawn * 4) + (black_aggr_safe_cnt * 6)
        ...
        # if self.get_max_player() == constant.WHITE:
        #     if self.evals[white_piece_eval - black_piece_eval] == 0:
        #         return white_piece_eval - black_piece_eval
        #     else:
        #         lookup_key = white_piece_eval - black_piece_eval
        #         while self.evals[lookup_key] > 0 and lookup_key > 0:
        #             lookup_key = lookup_key / 2
        #         return lookup_key
        # else:
        #     if self.evals[black_piece_eval - white_piece_eval] == 0:
        #         return black_piece_eval - white_piece_eval
        #     else:
        #         lookup_key = black_piece_eval - white_piece_eval
        #         while self.evals[lookup_key] > 0 and lookup_key > 0:
        #             lookup_key = lookup_key / 2
        #         return lookup_key
    ...
    def setup_prev_evals(self):
        self.evals.clear()
        if os.path.isfile('playdata.txt'):
            ifp = open(os.path.join(os.getcwd(), 'playdata.txt'), 'r')
            lines = ifp.readlines()
            for line in lines:
                self.evals[float(line.rstrip())] += 1

    def alphabeta(self, row: int, col: int, is_jump_pending: bool, depth: int, is_max_player: bool, alpha: float, beta: float) -> tuple:
        if (depth == constant.MAX_DEPTH) or (self.is_winner_found()):
